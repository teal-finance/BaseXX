...
type testValues struct {
	dec []byte
	enc string
	en2 []byte // to compare to a different encoder using a different alphabet
}
...
func BenchmarkEncodeACBase91(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		acBase91.Encode(testPairs[i%n].dec)
	}
}
...
func BenchmarkDecodeACBase91(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_ = acBase91.Decode(testPairs[i%n].en2)
	}
}
